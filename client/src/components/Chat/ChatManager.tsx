...
const ChatManager: FC<ChatManagerProps> = () => {
  ...
  useEffect(() => {
    const fetchChatHistory = async (): Promise<void> => {
      try {
        // Ensure user ID is a valid number, with multiple fallback strategies
        let userId = '0';
        ...
        // Strategy 2: Try generating a unique ID if no stored ID
        if (userId === '0') {
        ...
        let serverChatHistory: ChatMessage[] = [];
        try {
